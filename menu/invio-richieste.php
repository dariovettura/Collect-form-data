...
function richieste_options_page() {
  ...
  if ($categories):
      echo '<select id="category-select" class="custom-select">';
      // Aggiungi un'opzione placeholder
      echo '<option value="">' . esc_html__('Seleziona categoria', 'textdomain') . '</option>';
      foreach ($categories as $category):
          echo '<option value="' . esc_attr($category['name']) . '">' . esc_html($category['label']) . '</option>';
      endforeach;
      echo '</select>';
  endif;
  ...
  echo '</div>';
  // Aggiungi un titolo per la lista dei fornitori
  echo '<h2>Lista fornitori per categoria</h2>';
...
function get_fornitori_by_category() {
...
function invia_email_fornitori() {
  ...
  $body_message = get_post_field('post_content', $preventivo_id);
    ...
    // Recupera l'allegato dal post preventivo
    $allegato_id = get_post_meta($preventivo_id, '_allegato', true);

    // Verifica se l'allegato è un'immagine o un file
    if ($allegato_id) {
    ...
    // Costruisci l'allegato per l'email
    $attachments = array();
...
function enqueue_custom_admin_scripts_richieste($hook)
{
  ...
  wp_localize_script('richieste-admin-js', 'fornitori_ajax_object', array(
